...
function(x, y, mar = c(1, 1, 2, 1), ...)
{
    ...
    on.exit(par(opar))
    par(mfrow = c(r, c), mar = mar)
    invisible(mapply(function(p, i, ...) plot(p, main = paste0("Page ", i), ...), getPages(x), seq(1, np), MoreArgs = list(...)))
})

#XXX Compute colors
# pageTitle() function and default value.
setMethod("plot", "DocumentPage",
function(x, y, colors = getTextColors(x), ...)
{
    #?? Should this be
    ...
    plot(bb, pageHeight =  getPageHeight(x), colors = colors, ...)
})

setMethod("plot", "TextBoundingBox",
function(x, y, pageHeight = max(bottom(x)), colors = getTextColors(x), ...)
{    
    plot(1, xlim = range(c(left(x), right(x))), ylim = range(0, pageHeight), ..., xlab = "", ylab = "", axes = FALSE)
    box()
